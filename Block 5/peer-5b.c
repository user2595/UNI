...
#define TIMEOUT 50   // for the best tutor of them all

#define LISTEN_BACKLOG 21000   // Amount of clients 'listen' can handle waiting at once
...
typedef struct fingertable_element {
	uint16_t id;
	unsigned long ip;
	int port;
}fingertable_element;
...
int main(int argc, char *argv[]) {
    ...
    fingertable_element fingertable[16];	// 16te könnte unnötig sein
...
// convert my portno. to uint8_t
	...
	// set fingertable
	for (int i = 0; i < 16; ++i) {
		fingertable_element new_element;
		new_element.id = ((uint16_t) (my_id + pow(2, i)) % (256*256)); 
		new_element.ip = -1;
		new_element.port = -1;
		fingertable[i] = new_element;
        printf("%sFINGERTABLE: id = %d  ip = %lu  port = %d\n", KRED, i, fingertable[i].ip, fingertable[i].port);
        printf("%s", KWHITE);
	}
...
// __________________________________________________ MY JOIN REQUEST ________________________________________
    ...
    // set fingertable
            for(int i = 0; i < 16; i++) {
                fingertable[i].ip = ((unsigned long)header_new[2] << 24) + ((unsigned long)header_new[3] << 16) + ((unsigned long)header_new[4] << 8) + ((unsigned long)header_new[5]);
                fingertable[i].port = ((int)header_new[6] << 8) + ((int)header_new[7]);
                printf("%sFINGERTABLE: id = %d  ip = %lu  port = %d\n", KRED, i, fingertable[i].ip, fingertable[i].port);
                printf("%s", KWHITE);
            }
...
// __________________________________________________ RECEIVE HEADER ________________________________________
    ...
    // evaluate header
        ...
        if((flag_intern > 0) || ((header[0] & ACKNOLEDGE) == ACKNOLEDGE)) {
            flag_intern = TRUE;
        }
...
// __________________________________________________ RETURN FROM PEER & STABILIZE ________________________________________
    ...
    //  Set Internbit 0, send back to Client from Hash_Client!
        if(flag_intern == TRUE && (flag <= GET || (flag > NOTIFY && flag <= NOTIFY+GET))  && (header[0] & ACKNOLEDGE) == ACKNOLEDGE ) {  
            printf("Case 2.2 - Return from Peer to Client (Direct)\n"); // #DEBUG

            header[0] = header[0] & (0x8F);  //set Internbit to 0
            if(flag > NOTIFY) {
                header[0] -= (NOTIFY+ACKNOLEDGE);
            }
            hashtable_print_client(); // #DEBUG

        // update fingertable
            uint16_t temp_id = ((uint16_t) header_new[0] << 8) + header_new[1];
            uint16_t temp_hash = hash(key_buffer);
            for(int i = 0; i < 16; i++) {
            	if(temp_hash <= fingertable[i].id && fingertable[i].id <= temp_id) {
            		fingertable[i].ip = ((unsigned long)header_new[2] << 24) + ((unsigned long)header_new[3] << 16) + ((unsigned long)header_new[4] << 8) + ((unsigned long)header_new[5]);
            		fingertable[i].port = ((int)header_new[6] << 8) + ((int)header_new[7]);
            	}
            	else if(temp_hash > temp_id && fingertable[i].id <= temp_id) {
            		fingertable[i].ip = ((unsigned long)header_new[2] << 24) + ((unsigned long)header_new[3] << 16) + ((unsigned long)header_new[4] << 8) + ((unsigned long)header_new[5]);
            		fingertable[i].port = ((int)header_new[6] << 8) + ((int)header_new[7]);
            	}
            	else if(temp_hash > temp_id && fingertable[i].id >= temp_hash) {
            		fingertable[i].ip = ((unsigned long)header_new[2] << 24) + ((unsigned long)header_new[3] << 16) + ((unsigned long)header_new[4] << 8) + ((unsigned long)header_new[5]);
            		fingertable[i].port = ((int)header_new[6] << 8) + ((int)header_new[7]);
            	}
            }
...
// __________________________________________________ INCOMING NOTIFY ________________________________________
    ...
    // Errorcheck
        if(flag == NOTIFY && flag_connecttype == PREDECESSOR) {
            ...
            // set fingertable
                for(int i = 0; i < 16; i++) {
                    fingertable[i].ip = ((unsigned long)predecessor_ip[0] << 24) + ((unsigned long)predecessor_ip[1] << 16) + ((unsigned long)predecessor_ip[2] << 8) + ((unsigned long)predecessor_ip[3]);
                    fingertable[i].port = ((int)predecessor_port[0] << 8) + ((int)predecessor_port[1]);
                    printf("%sFINGERTABLE: id = %d  ip = %lu  port = %d\n", KRED, i, fingertable[i].ip, fingertable[i].port);
                    printf("%s", KWHITE);
                }
...
// Case 1: nicht Empfänger und nicht Hash_Verantwortlicher
        // Send an Peer aus Fingertable

        if(flag_intern == TRUE && flag_responsible == FALSE && ((uint16_t)header_new[0] << 8) + header_new[1] != my_id && flag <= GET) {
            printf("CASE 1\n"); // #DEBUG
            header[0] = header[0] | (INTERN);

        // find peer from fingertable
            int i = 0;
            while(fingertable[i].id > hash(key_buffer) && i < 16) {
                i++;
            }
            while(fingertable[i].id <= hash(key_buffer) && i < 16) {
                i++;
        	...
        	peer_ip = fingertable[i-1].ip;
            peer_ip = htonl(peer_ip);
            inet_ntop(AF_INET, &peer_ip, peer_ip_string, INET_ADDRSTRLEN); // #ERROR
            peer_port = fingertable[i-1].port;
            itoa(peer_port, peer_port_string);
...
// Case 2.1: Empfänger aber nicht Hash_Verantwortlicher
        // Sende an Peer aus Fingertable, speicher Client in Hash_Client
...
// Case 3: nicht Empfänger aber Hash_Verantwortlicher
        ...
        // instead of sending no acknoledge-bit, we send extra notify bit
            if ((header[0] & ACKNOLEDGE) == 0) {
            	header[0] += ACKNOLEDGE;	// set acknoledge-bit
            	header[0] += NOTIFY; 
            }
