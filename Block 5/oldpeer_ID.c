...
int send_all (int socket, uint8_t *header, uint8_t *header_2, uint8_t *key, uint8_t *value, uint8_t flag, uint8_t flag_intern) {
        ...
        printf("    Header_2: | %d | %d | %d | %d | %d | %d | %d |\n\n", header_2[0], header_2[1], header_2[2], header_2[3], header_2[4], header_2[5], header_2[6]);
...
// #ERROR
void set_headernew(uint8_t* header_new, uint8_t id, uint8_t ip[], uint8_t port[]) {
...
int main(int argc, char *argv[]) {
    ...
    uint8_t header[6] = {0}, header_new[7];                   // storage arrays for 'header_old' and 'header_new'
    uint8_t my_id, my_ip[4], my_port[2];         // uint8 ID values
    uint8_t predecessor_id = 0, predecessor_ip[4] = {0}, predecessor_port[2] = {0};
    uint8_t successor_id = 0, successor_ip[4] = {0}, successor_port[2] ={0};
    long my_ip_long;
...
// errorcheck : argument count
    ...
    // #TODO : HANDLE INCOMING JOIN OPERATION, DO A STABILIZE OPERATION, IMPLEMENT A NOTIFY MESSAGING OPERATION
    //          DEAL WITH SUCCESOR CLOSING SOCKET AFTER MISMATCHING STABILIZE
    //          AFTERWARDS CHANGE ID TO UINT16_T
...
// seperate different argument-cases -> contact peer or leave 'sock_successor' = 0, also set 'my_id'
    if(argc == 3) {
        ...
        my_id = (uint8_t) strtol(argv[3], (char **)NULL, 10);
    }
...
// __________________________________________________ MY JOIN REQUEST ________________________________________
    ...
    // receive answer to join request  
        ...
        if(receive_bytes(7, sock_successor, header_new) == -1) {
    ...
    // errorcheck
        ...
        successor_id = header_new[0];
        successor_ip[0] = header_new[1];
        successor_ip[1] = header_new[2];
        successor_ip[2] = header_new[3];
        successor_ip[3] = header_new[4];
        successor_port[0] = header_new[5];
        successor_port[1] = header_new[6];
...
// __________________________________________________ RECEIVE HEADER ________________________________________
    ...
    // receive or set the second part of header
        ...
        // receive second part in 'header_new'
            if(receive_bytes(7, sock_connector, header_new) == -1) {
                perror("Receive_Header failed");
                break;
            }
        }
...
// __________________________________________________ RETURN FROM PEER & STABILIZE ________________________________________
    ...
    //  Set Internbit 0, send back to Client from Hash_Client!
        if(flag_intern == TRUE && (flag <= GET) && header_new[0] == my_id) {  // #ERROR : Flag inconsistancies
    ...
    // Incoming STABILIZE
        if(flag == STABILIZE) {
            ...
            if(sock_predecessor < 0) {     // we dont have a predecessor yet, adopt connector as predecessor
                ...
                predecessor_id = header_new[0];
                predecessor_ip[0] = header_new[1];
                predecessor_ip[1] = header_new[2];
                predecessor_ip[2] = header_new[3];
                predecessor_ip[3] = header_new[4];
                predecessor_port[0] = header_new[5];
                predecessor_port[1] = header_new[6];
...
// __________________________________________________  JOIN - FIRST PART  ________________________________________
        ...
        if(flag == JOIN && flag_connecttype == DIRECT && header_new[0] == my_id) {
    ...
    // Check if we are responsible by using key_hashvalue
            ...
            //printf("Find responsible for Joiner\n");
            key_hashvalue = header_new[0];
...
// __________________________________________________  JOIN - SECOND PART  ________________________________________
        ...
        // Incoming JOIN
            ...
            // send to Predecessor my id/ip/port
...
// Case 1: nicht Empfänger und nicht Hash_Verantwortlicher
        ...
        if(flag_intern == TRUE && flag_responsible == FALSE && header_new[0] != my_id && flag <= GET) {
            printf("CASE 1\n"); // #DEBUG
...
// Case 3: nicht Empfänger aber Hash_Verantwortlicher
        ...
        // dont send key/value if they are not needed anymore
            ...
            if(header_new[5] == predecessor_port[0] && header_new[6] == predecessor_port[1]) {
