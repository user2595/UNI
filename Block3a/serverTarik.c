  ...
  while(1)
  {
        ...
        while(bytes_recv_all < header_size) {
            if((bytes_recv = recv(their_addr_id, &header[bytes_recv_all], 1, 0)) == -1) {
                ...
                printf("RECV: header[%d] = %d\n", bytes_recv_all,header[bytes_recv_all]);
        ...
        printf("Header[2] = %d\n", header[2]);
        printf("Header[3] = %d\n", header[3]);
        //THIS IS A HUGE PROBLEM!!!! HEADER[2] AND HEADER[3] SHOULD BE SWAPPED
        ...
        // Sendet den Header
        //THIS IS TO BE DELETED WHEN NOT USED FOR TESTING ANYMORE BECAUSE ITS BS AND SHOULD NOT BE THE SOLUTION
        ...
        //DELETE THIS BS ITS ONLY FOR TESTING
        for (int i = 0; i < 6; ++i)
        {
...
//___________________________// Tarik Version___________________________________________________
        ...
        if (flag == 4 )
        {
            ...
            if(s != NULL){   // nur wenn Value in der HH
                free(value_buffer);
                char* value_buffer = malloc(s-> value_len);
                strcpy(value_buffer, s->value);
                value_len = s->value_len;
                    if((b_send = send(their_addr_id,value_buffer , value_len-1, 0))==-1){
                        perror("Sending Failed");
                        close(sockfd);
                        close(their_addr_id);
                        exit(1);
...
//_____________________________________// Tarik Version_______________________________________________________
             //printf("KLen %ld\n", key_len);
            //printf("Value : %s\n", value_buffer);
            //printf("Vlen %ld\n", value_len);
            //printf("alter %s\n",s->value);
